...
# Set a random seed so that results are repeatable
set.seed(100) 
...
model_rf1 <- train(status~., data = train, method = 'rf', metric="ROC", trControl = control)
# top 5 results
model_rf1$results %>% 
  top_n(5, wt = ROC) %>%
  arrange(desc(ROC))
...
#  mtry  ROC        Sens       Spec     
#2    0.8166799  0.1533333  1.0000000

######################################

# Predict
test$status <- predict(model_rf1, newdata = test, type='prob')
...
write.csv(export,"./results/rf_1.csv", row.names = FALSE)
rm(export)
test$status <- NULL
...
model_rf2 <- train(status~., data = df_train, method = 'rf', metric="ROC", trControl = control)
print(model_rf2)
#  mtry  ROC        Sens       Spec     
...
model_rf3 <- train(status~., data = df_train_rfe, method = 'rf', metric="ROC", trControl = control)
print(model_rf3)
#  mtry  ROC        Sens       Spec     
...
model_rf4 <- train(status~., data = df_train_boruta, method = 'rf', metric="ROC", trControl = control)
print(model_rf4)
#  mtry  ROC        Sens       Spec     
...
model_rf6 <- train(status~., data=train, method=customRF, metric="ROC", tuneGrid=tunegrid, trControl=control)
print(model_rf6)

#  mtry  ntree  ROC        Sens        Spec     
#3    1500   0.8654126  0.24333333  0.9976601  **

######################################

# Predicting 
test$status <- predict(model_rf7, newdata = test, type = "prob")
...
#write.csv(export,"results/rf_2.csv", row.names = FALSE)
rm(export)
test$status <- NULL
...
model_rf7 <- train(status~., data=df_train_boruta, method=customRF, metric="ROC", tuneGrid=tunegrid, trControl=control)
print(model_rf7)
